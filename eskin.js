...
function resetFormEskalasi() {
    ...
    // Reset isi tabel item
    const tbody = document.getElementById("itemRows");
    if (tbody) tbody.innerHTML = "";
    
    // Reset counter baris & tombol Add Item
    rowCount = 0;
    ...
    // Re-fetch nomor eskalasi terbaru
    fetchNextNomor();
}
...
function closeModalEskalasi() {
    ...
    if (modal) {
        ...
        resetFormEskalasi(); // Reset form dan item saat modal ditutup / batal
    }
}
...
function tambahBaris() {
    ...
    tr.className = "hover:bg-[#f8f9fa] dark:hover:bg-[#252525] transition-colors";
    tr.innerHTML = `
        <td class="p-2">
        ...
        <td class="p-2">
            <input type="text" inputmode="numeric" class="item-cost w-full !bg-[#f1f3f4] dark:!bg-[#2d2d2d] !text-[#202124] dark:!text-[#e8eaed] border border-transparent dark:border-[#3c4043] rounded-xl px-3 py-1.5 text-xs text-right outline-none focus:border-[#1a73e8] dark:focus:border-[#8ab4f8] transition" placeholder="0" oninput="this.value = this.value.replace(/[^0-9]/g, ''); hitungTotal()">
        </td>
    `;
...
async function initEskin() {
    try {
        ...
        if (itemsData) {
            ...
            // Hapus panggilang tambahBaris() ganda saat init jika sudah ditangani resetFormEskalasi
