...
/**
 * FUNGSI EXPORT PDF SESUAI FILTER/SEARCH/SORT AKTIF
 * - Format: A4 Portrait
 * - Footer: Pojok kanan bawah "Exported by (nama akun)" (ukuran kecil)
 */
async function exportToPDF() {
  try {
    ...
    // Ambil seluruh data yang terfilter tanpa limit pagination
    let exportQuery = _supabase.from(TABLE_NAME).select('*');
    exportQuery = applySupabaseFilters(exportQuery);

    if (sortConfig.column && sortConfig.direction) {
      exportQuery = exportQuery.order(sortConfig.column, { ascending: sortConfig.direction === 'asc' });
    } else {
      exportQuery = exportQuery.order('Tanggal', { ascending: false });
    }
    ...
    doc.autoTable({
      ...
      columnStyles: {
        ...
        1: { cellWidth: 22 }, // Sumber
        2: { cellWidth: 44 }, // Case
        3: { cellWidth: 20 }, // PIC
        4: { cellWidth: 44 }, // Update
        5: { cellWidth: 18 }, // Target
      ...
      didDrawPage: function (data) {
        ...
        // Nomor Halaman di Pojok Kiri Bawah
