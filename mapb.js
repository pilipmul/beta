// ==========================================
// MAPB.JS - POP UP DETAIL INFORMASI TENANT & HSE
// ==========================================

/**
 * Helper untuk mengamankan string dari potensi XSS
 */
...
async function showTenantDetailPopup(noTenant) {
  ...
  try {
    const { data, error } = await supabaseClient
      .from('tenant')
      .select('no, lokasi, lantai, luas, tipe, penyewa, status, komoditi, cp, validasi')
      .eq('no', noTenant)
      .single();
...
/**
 * Mengambil detail lengkap item HSE dari Supabase dan menampilkan pop-up
 * @param {number|string} noHse - ID/No database HSE yang diklik
 */
async function showHseDetailPopup(noHse) {
  if (typeof showHSEDetailPopup === 'function') {
    return showHSEDetailPopup(noHse);
  }
}
