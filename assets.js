...
// INISIALISASI HEADER & HAMBURGER ITEMS
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Realtime listener khusus UPDATE & Perubahan Data pada tabel assets
  _supabase
...
function handleRealtimeUpdate(newRow) {
  ...
  const photoBtn = newRow.link 
    ? `<a href="${escapeHtml(newRow.link)}" target="_blank" rel="noopener noreferrer" class="inline-flex items-center gap-1 px-2.5 py-1 bg-[#e8f0fe] dark:bg-[#2c384e] hover:bg-[#d2e3fc] dark:hover:bg-[#3a4963] text-[#1a73e8] dark:text-[#8ab4f8] rounded-full border border-[#1a73e8]/20 dark:border-[#8ab4f8]/20 font-medium text-[11px] transition cursor-pointer">
...
document.addEventListener('click', (e) => {
  ...
  if (!e.target.closest('#inputPic') && !e.target.closest('#picSuggestList')) {
    document.getElementById('picSuggestList').classList.add('hidden');
  }
});
...
function renderTable() {
  ...
  fetchedData.forEach(row => {
    ...
    const photoBtn = row.link 
      ? `<a href="${escapeHtml(row.link)}" target="_blank" rel="noopener noreferrer" class="inline-flex items-center gap-1 px-2.5 py-1 bg-[#e8f0fe] dark:bg-[#2c384e] hover:bg-[#d2e3fc] dark:hover:bg-[#3a4963] text-[#1a73e8] dark:text-[#8ab4f8] rounded-full border border-[#1a73e8]/20 dark:border-[#8ab4f8]/20 font-medium text-[11px] transition cursor-pointer">
          <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z"/></svg>
          Foto
         </a>`
      : `<span class="text-[#5f6368] dark:text-[#9aa0a6] text-[11px]">-</span>`;
