...
function updateFilterButtonStyles() {
  const allColumns = ['no', 'lokasi', 'luas', 'tipe', 'penyewa', 'status', 'cp', 'validasi'];
...
async function fetchTableData() {
  ...
  try {
    ...
    if (totalRows === 0) {
      tbody.innerHTML = `<tr><td colspan="8" class="p-6 text-center text-[#5f6368] dark:text-[#9aa0a6]">Data tenant tidak ditemukan.</td></tr>`;
      updatePaginationUI();
...
function renderTable() {
  ...
  fetchedData.forEach(row => {
    ...
    if (statusVal.includes('tersewa') || statusVal.includes('active') || statusVal.includes('isi')) {
...
async function exportCSV() {
  try {
    ...
    const headers = ['no', 'lokasi', 'lantai', 'luas', 'tipe', 'penyewa', 'status', 'komoditi', 'cp', 'validasi'];
