...
// ==========================================
// ==========================================
// LANTAI & CATEGORY CONTROLLER (FIXED)
// ==========================================
function switchFloor(floorKey) {
  ...
  ['basement', 'dasar', 'satu', 'dua', 'rooftop'].forEach(f => {
    ...
    if (tab) {
      tab.className = (f === floorKey)
        ? "px-2 sm:px-2.5 py-0.5 rounded-md font-medium transition bg-[#1a73e8] dark:bg-[#8ab4f8] text-white dark:text-[#121212] shadow-xs cursor-pointer"
        : "px-2 sm:px-2.5 py-0.5 rounded-md font-medium transition text-[#5f6368] dark:text-[#9aa0a6] hover:text-[#202124] dark:hover:text-white cursor-pointer";
    }
  });
...
function setActiveCategory(cat) {
  ...
  if (cat === 'tenant') {
    if (btnTenant) btnTenant.className = "px-2.5 py-0.5 rounded-md font-medium transition bg-blue-600 text-white cursor-pointer flex items-center gap-1.5";
    if (btnHse) btnHse.className = "px-2.5 py-0.5 rounded-md font-medium transition text-[#5f6368] dark:text-[#9aa0a6] hover:text-[#202124] dark:hover:text-white cursor-pointer flex items-center gap-1.5";
    
    // Tampilkan layer Tenant, sembunyikan HSE
    if (tLayer) tLayer.style.display = 'block';
    if (hLayer) hLayer.style.display = 'none';
  } else {
    if (btnHse) btnHse.className = "px-2.5 py-0.5 rounded-md font-medium transition bg-emerald-600 text-white cursor-pointer flex items-center gap-1.5";
    if (btnTenant) btnTenant.className = "px-2.5 py-0.5 rounded-md font-medium transition text-[#5f6368] dark:text-[#9aa0a6] hover:text-[#202124] dark:hover:text-white cursor-pointer flex items-center gap-1.5";
    
    // Sembunyikan layer Tenant, tampilkan HSE
...
function setAppMode(mode) {
  ...
  // Pastikan render terpanggil begitu pindah mode
  refreshActiveModule();
}
...
function setActiveCategory(cat) {
  ...
  if (cat === 'tenant') {
    btnTenant.className = "px-2.5 py-0.5 rounded-md font-medium transition bg-blue-600 text-white cursor-pointer flex items-center gap-1.5";
    btnHse.className = "px-2.5 py-0.5 rounded-md font-medium transition text-[#5f6368] dark:text-[#9aa0a6] hover:text-[#202124] dark:hover:text-white cursor-pointer flex items-center gap-1.5";
    
    // Sembunyikan layer HSE, tampilkan layer Tenant
    const tLayer = document.getElementById('marker-tenant-layer');
    const hLayer = document.getElementById('marker-hse-layer');
    if (tLayer) tLayer.style.display = 'block';
    if (hLayer) hLayer.style.display = 'none';
  } else {
    btnHse.className = "px-2.5 py-0.5 rounded-md font-medium transition bg-emerald-600 text-white cursor-pointer flex items-center gap-1.5";
    btnTenant.className = "px-2.5 py-0.5 rounded-md font-medium transition text-[#5f6368] dark:text-[#9aa0a6] hover:text-[#202124] dark:hover:text-white cursor-pointer flex items-center gap-1.5";
    
    // Sembunyikan layer Tenant, tampilkan layer HSE
    const tLayer = document.getElementById('marker-tenant-layer');
    const hLayer = document.getElementById('marker-hse-layer');
    if (tLayer) tLayer.style.display = 'none';
    if (hLayer) hLayer.style.display = 'block';
