...
function handleRealtimeUpdate(newRow) {
  ...
  cellValues.forEach(col => {
    ...
    if (cell) {
      if (cell.innerHTML !== String(col.html)) cell.innerHTML = col.html;
      if (col.title !== undefined) cell.title = col.title;
    }
  });
}
...
async function preloadSuggestions() {
  try {
    ...
    if (usersRes.data) dbUsersList = Array.from(new Set(usersRes.data.map(u => u.nama).filter(Boolean)));
    if (itemsRes.data) dbItemsList = Array.from(new Set(itemsRes.data.map(i => i.item).filter(Boolean)));
  } catch (err) {
...
function updateFilterButtonStyles() {
  const allColumns = ['no', 'user', 'item', 'in', 'out', 'date', 'note', 'trx_code', 'balance', 'project'];
...
async function saveData(e) {
  ...
  const payload = { user, item, in: inVal, out: outVal, date, trx_code, note };

  try {
    if (no) {
      const { error } = await _supabase.from('inventory').update(payload).eq('no', no);
      if (error) throw error;
    } else {
...
async function deleteDataInModal() {
  ...
  try {
    const { error } = await _supabase.from('inventory').delete().eq('no', no);
    if (error) throw error;
...
async function openExportModal() {
  ...
  try {
    const { data, error } = await _supabase.from('inventory').select('item').not('item', 'is', null);
    if (error) throw error;

    dbExportItemsList = Array.from(new Set(data.map(i => i.item).filter(Boolean))).sort();
    selectedExportItems = [...dbExportItemsList];

    renderExportItemCheckboxes();
...
function renderExportItemCheckboxes() {
  ...
  const itemList = dbExportItemsList;
  ...
  itemList.forEach(item => {
    ...
    checkbox.className = 'export-item-cb filter-cb';
...
function handleExportItemCbChange(cb) {
  ...
  const allCount = dbExportItemsList.length;
  document.getElementById('exportItemCount').innerText = `Terpilih: ${selectedExportItems.length}/${allCount}`;
  document.getElementById('exportSelectAllCb').checked = (selectedExportItems.length === allCount);
}

function toggleSelectAllExportItems(checked) {
  selectedExportItems = checked ? [...dbExportItemsList] : [];
  document.querySelectorAll('.export-item-cb').forEach(cb => { cb.checked = checked; });
