...
function renderTable() {
  ...
  fetchedData.forEach((row, index) => {
    ...
    tr.innerHTML = `
      ...
      <td class="p-2.5 font-semibold text-[#202124] dark:text-[#f1f3f4]">${escapeHtml(row.subject || '-')}</td>
      <td class="p-2.5 font-medium">${escapeHtml(row.item || '-')}</td>
      <td class="p-2.5 whitespace-nowrap">${formatDate(row.date)}</td>
      <td class="p-2.5 truncate">${escapeHtml(row.notes || '-')}</td>
