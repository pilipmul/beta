...
// Path relatif fleksibel (cukup letakkan gambar di folder yang sama)
const FLOOR_MAPS = {
...
// Daftar nama/username yang diizinkan untuk Edit Mode & Plotting
...
function renderHamburgerMenuContent() {
  ...
  if (allowEdit) {
    ...
    // Tampilan jika pengguna di luar daftar akun edit mode (Read Only)
    html += `
...
function setAppMode(mode) {
  // Blokir jika akun di luar ALLOWED_EDITORS mencoba masuk ke edit mode
...
function selectTenant(no) {
  ...
  if (tenant) {
    ...
    if (tenant.koordinat && tenant.koordinat.x != null && tenant.koordinat.y != null) {
      // Panggil fungsi pop up dari mapb.js
...
function openModal(mode, no = null) {
  if (!isSuperAdmin()) {
    alert("Akses Terbatas: Anda tidak memiliki izin untuk menambah atau mengubah data tenant.");
    return;
  }
