...
function getStatusClass(status) {
  ...
  if (s.includes('hilang')) return 'status-hilang';
...
async function fetchDataFromSupabase() {
  try {
    const { data, error } = await supabaseClient
      .from('tenant')
      .select('no, lokasi, lantai, koordinat, status')
...
function switchFloor(floorKey) {
  ...
  ['basement', 'dasar', 'satu', 'dua', 'rooftop'].forEach(f => {
    ...
    if (tab) {
      if (f === floorKey) {
        tab.className = "px-2 sm:px-2.5 py-0.5 rounded-md font-medium transition bg-[#1a73e8] dark:bg-[#8ab4f8] text-white dark:text-[#121212] shadow-xs cursor-pointer";
      } else {
        tab.className = "px-2 sm:px-2.5 py-0.5 rounded-md font-medium transition text-[#5f6368] dark:text-[#9aa0a6] hover:text-[#202124] dark:hover:text-white cursor-pointer";
      }
    }
  });
  ...
  contentContainer.innerHTML = `
    <div class="flex items-start justify-between border-b border-slate-200 dark:border-zinc-800 pb-2 mb-3">
      <div class="pr-2 min-w-0">
        <span class="text-[10px] font-bold text-blue-600 dark:text-blue-400 uppercase tracking-wider block">Lantai ${escapeHtml(tenant.lantai)}</span>
        <h3 class="text-sm font-bold text-slate-800 dark:text-slate-100 leading-snug mt-0.5 truncate" title="${escapeHtml(tenant.lokasi)}">${escapeHtml(tenant.lokasi)}</h3>
      </div>
      <button onclick="closeFloatingCard()" class="text-slate-400 hover:text-slate-600 dark:hover:text-white p-1 rounded-full text-xs transition cursor-pointer">✕</button>
    </div>
...
function initMapControls() {
  ...
  viewport.addEventListener('click', (e) => {
    if (!isPlacingMode || !selectedTenantNo || appMode !== 'edit' || !isSuperAdmin()) return;
...
function openModal(mode, no = null) {
  if (!isSuperAdmin()) {
    alert("Akses Terbatas: Anda tidak memiliki izin untuk mengedit data tenant.");
    return;
  }
...
async function exportToPDF() {
  ...
  try {
    // 1. Ambil ukuran asli gambar denah
    const imgW = denahImg.naturalWidth || denahImg.width || 800;
    const imgH = denahImg.naturalHeight || denahImg.height || 600;

    // 2. Buat element canvas internal
    const canvas = document.createElement('canvas');
    const scaleFactor = 2; // Presisi tinggi
    canvas.width = imgW * scaleFactor;
    canvas.height = imgH * scaleFactor;
    const ctx = canvas.getContext('2d');
    ctx.scale(scaleFactor, scaleFactor);

    // 3. Render gambar denah langsung dari elemen <img> yang sudah diload di DOM
    ctx.drawImage(denahImg, 0, 0, imgW, imgH);

    // 4. Gambar seluruh marker yang aktif di lantai ini secara manual
    const floorTenants = tenantsData.filter(t => t.lantai === currentFloor);
    ...
    floorTenants.forEach(t => {
      ...
      // Tentukan warna sesuai status (RGBA Stabilo)
      let fillColor = 'rgba(239, 68, 68, 0.4)';  // Merah (Hilang/Default)
      let strokeColor = '#dc2626';

      if (status.includes('sewa')) {
        fillColor = 'rgba(59, 130, 246, 0.4)';   // Biru
        strokeColor = '#2563eb';
      } else if (status.includes('kosong')) {
        fillColor = 'rgba(249, 115, 22, 0.4)';  // Oranye
        strokeColor = '#ea580c';
      } else if (status.includes('terjual')) {
        fillColor = 'rgba(34, 197, 94, 0.4)';   // Hijau
        strokeColor = '#16a34a';
      } else if (status.includes('fasum')) {
        fillColor = 'rgba(6, 182, 212, 0.4)';   // Biru Muda
        strokeColor = '#0891b2';
      }

      // Draw Marker Lingkaran
      ctx.beginPath();
    ...
    const fileName = `Denah_Layout_${currentFloor.toUpperCase()}_${new Date().toISOString().slice(0, 10)}.pdf`;
    pdf.save(fileName);

  } catch (err) {
    console.error("Gagal membuat PDF layout:", err);
    alert('Gagal mengunduh PDF layout: ' + err.message);
  }
}

// Helper membaca file gambar lokal menjadi Base64 string
