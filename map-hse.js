...
async function fetchHSEFromSupabase() {
  try {
    ...
    if (typeof calculateHSECounts === 'function') calculateHSECounts();
...
function renderHSEMarkers() {
  ...
  floorHSE.forEach(h => {
    ...
    marker.className = `hse-marker absolute transform -translate-x-1/2 -translate-y-1/2 rounded-full flex items-center justify-center text-white shadow-md cursor-pointer transition-all ${config.bgColor} ${isSelected ? 'ring-4 ring-white shadow-xl scale-110 z-30' : 'hover:scale-115 z-20'} ${!isMatch ? 'is-filtered-out' : ''}`;
  ...
  contentContainer.innerHTML = `
    ...
    <div class="flex items-start justify-between border-b border-slate-200 dark:border-zinc-800 pb-2 mb-3">
      ...
      <button onclick="closeFloatingCard(); event.stopPropagation();" 
              class="w-7 h-7 sm:w-8 sm:h-8 flex items-center justify-center rounded-full bg-slate-100 dark:bg-zinc-800 text-slate-500 hover:text-slate-800 dark:text-slate-400 dark:hover:text-white hover:bg-slate-200 dark:hover:bg-zinc-700 transition cursor-pointer shrink-0 active:scale-95" 
              title="Tutup (Esc)">
        <i class="fa-solid fa-xmark text-sm sm:text-base pointer-events-none"></i>
      </button>
    </div>

    <!-- Detail Informasi HSE -->
