...
function renderHSEList() {
  ...
  // Menampilkan seluruh data HSE tanpa batasan lantai
      ...
      <div class="p-2.5 flex items-center justify-between transition-colors cursor-pointer hover:bg-slate-100 dark:hover:bg-zinc-800 ${isSelected ? 'bg-emerald-50 dark:bg-emerald-900/30 border-l-4 border-emerald-500' : ''}"
        ...
        <div class="flex-1 pr-2 min-w-0">
          <div class="flex items-center gap-2">
            <!-- IKON JENIS ALAT SEBAGAI PENGGANTI NOMOR URUT -->
...
function renderHSEMarkers() {
  ...
  // Hanya tampilkan marker yang diplot pada lantai aktif saat ini
...
function showHSEDetailPopup(noHSE) {
  ...
  const item = hseData.find(h => h.no === noHSE);

  if (!item || !floatingCard || !contentContainer) return;
  ...
  contentContainer.innerHTML = `
      ...
      <div class="pr-2 min-w-0">
        <div class="flex items-center gap-1.5 mb-0.5">
          ...
          <span class="text-[10px] font-bold text-slate-400 uppercase tracking-wider block">
            Lantai ${escapeHtml(item.lantai)}
          </span>
        </div>
        <h3 class="text-sm font-bold text-slate-800 dark:text-slate-100 leading-snug truncate" title="${escapeHtml(item.lokasi)}">
    ...
    <div class="space-y-2 text-xs text-slate-700 dark:text-slate-300 max-h-[60vh] sm:max-h-[70vh] overflow-y-auto custom-scroll pr-1">
      ...
      <div class="flex justify-between items-center py-1 border-b border-slate-100 dark:border-zinc-800">
        ...
        <span class="px-2 py-0.5 rounded-full text-[11px] font-semibold ${String(item.kondisi).toLowerCase().includes('baik') || String(item.kondisi).toLowerCase().includes('normal') ? 'bg-emerald-500/10 text-emerald-600 border border-emerald-500/30' : 'bg-red-500/10 text-red-600 border border-red-500/30'}">
      ...
      <div class="flex justify-between items-center py-1 border-b border-slate-100 dark:border-zinc-800">
        ...
        <span class="font-medium text-slate-800 dark:text-slate-200 truncate max-w-[150px]" title="${escapeHtml(item.keterangan)}">${escapeHtml(item.keterangan)}</span>
      </div>
